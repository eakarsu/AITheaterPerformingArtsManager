...
export default function CfCommunityPartnershipMatcherIdentifyingSponsorsByAffinity() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-community-partnership-matcher-identifying-sponsors-by-affinity/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-community-partnership-matcher-identifying-sponsors-by-affinity', project: 'AITheaterPerformingArtsManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-community-partnership-matcher-identifying-sponsors-by-affinity/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Community partnership matcher identifying sponsors by affinity</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITheaterPerformingArtsManager</p>
      </div>
