...
export default function CfVolunteerManagementModuleWithShiftScheduling() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-volunteer-management-module-with-shift-scheduling/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-volunteer-management-module-with-shift-scheduling', project: 'AITheaterPerformingArtsManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-volunteer-management-module-with-shift-scheduling/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Volunteer management module with shift scheduling</h1>
