...
export default function CfFundraisingCampaignPlanningWithDonorSegmentTargeting() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-fundraising-campaign-planning-with-donor-segment-targeting/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-fundraising-campaign-planning-with-donor-segment-targeting', project: 'AITheaterPerformingArtsManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-fundraising-campaign-planning-with-donor-segment-targeting/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Fundraising campaign planning with donor-segment targeting</h1>
