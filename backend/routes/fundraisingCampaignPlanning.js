...
// Fundraising campaign planning with donor-segment targeting
// Feature: fundraising-campaign-planning
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Fundraising campaign planning with donor-segment targeting.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'fundraising-campaign-planning', payload, result);
    res.json({ success: true, feature: 'fundraising-campaign-planning', ...result });
  } catch (err) {
    console.error('fundraising-campaign-planning analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Fundraising campaign planning with donor-segment targeting. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'fundraising-campaign-planning', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'fundraising-campaign-planning']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'fundraising-campaign-planning', title: 'Fundraising campaign planning with donor-segment targeting', project: 'AITheaterPerformingArtsManager' });
});
