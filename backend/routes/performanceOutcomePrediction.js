...
// Performance outcome prediction for ticket sales and review sentiment
// Feature: performance-outcome-prediction
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AITheaterPerformingArtsManager" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Performance outcome prediction for ticket sales and review sentiment.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'performance-outcome-prediction', payload, result);
    res.json({ success: true, feature: 'performance-outcome-prediction', ...result });
  } catch (err) {
    console.error('performance-outcome-prediction analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AITheaterPerformingArtsManager". Return strict JSON only.',
      `Feature: Performance outcome prediction for ticket sales and review sentiment. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'performance-outcome-prediction', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'performance-outcome-prediction']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'performance-outcome-prediction', title: 'Performance outcome prediction for ticket sales and review sentiment', project: 'AITheaterPerformingArtsManager' });
});
