...
// Community partnership matcher identifying sponsors by affinity
// Feature: community-partnership-matcher
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Community partnership matcher identifying sponsors by affinity.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'community-partnership-matcher', payload, result);
    res.json({ success: true, feature: 'community-partnership-matcher', ...result });
  } catch (err) {
    console.error('community-partnership-matcher analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Community partnership matcher identifying sponsors by affinity. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'community-partnership-matcher', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'community-partnership-matcher']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'community-partnership-matcher', title: 'Community partnership matcher identifying sponsors by affinity', project: 'AITheaterPerformingArtsManager' });
});
